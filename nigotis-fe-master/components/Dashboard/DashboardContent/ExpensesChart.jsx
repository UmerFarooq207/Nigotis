  ...
  if (active) {
    return (
      <div className="bg-white border border-gray-100 rounded-xl shadow-xl p-3 text-sm min-w-[160px] z-50">
        <p className="font-semibold text-gray-700 mb-2">Expenses Breakdown</p>
        {allData.map((entry) => (
...
export default function ExpensesChart() {
  ...
  const fallbackData = {
    ...
    datasets: [{ data: [30, 25, 25], backgroundColor: ["#ef4444", "#991b1b", "#fca5a5"], borderWidth: 0, hoverOffset: 4 }],
  };
  ...
  const chartData = data?.chartData
    ? {
        ...
        datasets: data.chartData.datasets?.map((ds) => ({
          ...ds,
          backgroundColor: ["#dc2626", "#7f1d1d", "#f87171", "#fee2e2"],
        })),
      }
    : fallbackData;
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500">Spending for {getPeriodName()}</p>
                <p className="text-2xl font-bold text-gray-900 mt-0.5">
                  {currency?.symbol}&nbsp;{getFormattedFigures(data?.totalExpenses)}
                </p>
              </div>
