...
export default function ItemList({ items, setItems }) {
  ...
  const hasShownNoProductsToastRef = useRef(false);
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (data?.success) {
        hasShownNoProductsToastRef.current = false;
        setProducts(data?.data);
      } else {
        setProducts([]);
        const message = data?.message || "No products found. Please add a product first.";
        if (
          !hasShownNoProductsToastRef.current ||
          !message.toLowerCase().includes("no products found")
        ) {
          hasShownNoProductsToastRef.current = message
            .toLowerCase()
