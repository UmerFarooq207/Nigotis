...
export async function POST(req) {
  try {
    ...
    if (!user_subscription) {
      return resError(
        "User subscription was not created. Please contact our team for help."
      );
    }
    console.log("user subscription ", user_subscription);
    ...
    if (user?.companyId) {
      ...
      company.subscriptionId = user_subscription?._id;
      await company.save();
    }
...
export async function PUT(req) {
  try {
    ...
    const subscription = await User_Subscription.findById(subscriptionId ? subscriptionId : authData1?.data?.subscriptionId);
    if (!subscription) {
      return resError(
