...
export async function DELETE(req) {
  try {
    ...
    const isSuperAdmin = authData2?.success;
    const authUser = isSuperAdmin ? authData2?.data : authData1?.data;

    const targetCompanyId =
      companyId || authData1?.data?.companyId?._id || authData1?.data?.companyId;

    if (!targetCompanyId) {
      return resError("Company id is required.");
    }

    if (
      !isSuperAdmin &&
      authData1?.data?.companyId &&
      String(targetCompanyId) !== String(authData1?.data?.companyId?._id)
    ) {
      return resError("You are not allowed to delete this company.");
    }

    let company = await Company.findById(targetCompanyId);
    if (!company) {
      return resError(`Company was not found against id: `, targetCompanyId);
    }

    await Company.findByIdAndDelete(targetCompanyId);

    if (company?.adminId) {
      await User.findByIdAndUpdate(company.adminId, {
        $unset: { companyId: "" },
      });
