...
import { subscriptionCheckGuard, userFinanceGuard } from "@/middleware/user";
...
authData?.data
    ...
    const { assetId, title, desc, quantity, totalAmount, date } =
      await req.json();
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Company asset has been updated successfully.",
        data: asset,
      },
      { status: 200 },
    );
