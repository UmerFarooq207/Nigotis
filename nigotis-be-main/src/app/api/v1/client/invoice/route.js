...
export async function POST(req) {
  try {
    ...
    if (!issueDate || !dueDate || !clientId || !status || items.length === 0) {
      return resError("One or more required fields are missing.");
    }

    let client = await Client.findById(clientId).populate({
      path: "personalInfo",
    });
    if (!client) {
    ...
    let mailData = {
      ...
      invoice,
      subject: `Invoice Generated | ${authData?.data?.companyId?.displayName} | Nigotis`,
      message: "Thank you for your order. Please find the invoice attached.",
      companyName: authData?.data?.companyId?.displayName,
      supportEmail: authData?.data?.companyId?.email,
      attachments: fileName
        ? [
            {
              filename: `Invoice-${invoice?.invoiceNo}.pdf`,
              path: `${process.env.NEXT_PUBLIC_AWS_OBJECT_BASE_URL}${fileName}`,
            },
          ]
        : [],
    };
    ...
    await createNotification({
      sentTo: authData?.data?.companyId?.adminId,
      title: "New Invoice Generated",
      message: `${
        updatedData?.invoiceBy?.personalInfo?.title
          ? updatedData?.invoiceBy?.personalInfo?.title + ". "
          : ""
      } ${
        updatedData?.invoiceBy?.personalInfo?.firstName
      } generated an invoice (ID:${updatedData?.invoiceNo}) for ${
        mailData?.firstName
      } ${mailData?.lastName}`,
...
export async function PUT(req) {
  try {
    ...
    let client = await Client.findById(invoice?.clientId).populate({
      path: "personalInfo",
    });
    if (!client) {
    ...
    let mailData = {
      ...
      lastName: client?.personalInfo?.lastName,
      invoice,
      subject: `Invoice Updated | ${authData?.data?.companyId?.displayName} | Nigotis`,
      message:
        "Your invoice has been updated. Please find the invoice attached.",
      companyName: authData?.data?.companyId?.displayName,
      supportEmail: authData?.data?.companyId?.email,
      attachments: fileName
        ? [
    ...
    await createNotification({
      ...
      message: `${
        invoice?.invoiceBy?.personalInfo?.title
          ? invoice?.invoiceBy?.personalInfo?.title + ". "
          : ""
      } ${
        invoice?.invoiceBy?.personalInfo?.firstName
      }  updated an invoice (ID:${invoice?.invoiceNo}).`,
      link: "/dashboard/invoices",
    });
...
async function sendMailToCustomer(data) {
  try {
    ...
    const mailBody = `
<br>Hi <b>${data?.firstName} ${data?.lastName}!</b>

<br><br>${data?.message}
<br>Invoice #: ${data?.invoice?.invoiceNo}
<br>Additional Notes: ${data?.invoice?.notes}
<br>
<br>If you need any further information, feel free to reach out to us at <a href="mailto:${data?.supportEmail}">${data?.supportEmail}</a>.
<br><br>Best Regards, <br>${data?.companyName}
`;
